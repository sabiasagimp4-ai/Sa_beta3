...
internal sealed class SaColorCiliaShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaColorCiliaShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaColorCiliaShader).Assembly.GetManifestResourceStream("SaColorCilia.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
