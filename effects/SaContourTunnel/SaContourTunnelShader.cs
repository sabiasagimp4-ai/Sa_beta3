...
internal sealed class SaContourTunnelShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaContourTunnelShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaContourTunnelShader).Assembly.GetManifestResourceStream("SaContourTunnel.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
