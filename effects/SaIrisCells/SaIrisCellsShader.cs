...
internal sealed class SaIrisCellsShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaIrisCellsShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaIrisCellsShader).Assembly.GetManifestResourceStream("SaIrisCells.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
