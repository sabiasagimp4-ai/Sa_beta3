...
internal sealed class SaChromaticZipperShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaChromaticZipperShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaChromaticZipperShader).Assembly.GetManifestResourceStream("SaChromaticZipper.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
