...
internal sealed class SaPetalGraftShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaPetalGraftShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaPetalGraftShader).Assembly.GetManifestResourceStream("SaPetalGraft.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
