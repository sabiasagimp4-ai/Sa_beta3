...
internal sealed class SaFacetBloomShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaFacetBloomShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaFacetBloomShader).Assembly.GetManifestResourceStream("SaFacetBloom.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
