...
[VideoEffect("色境界の縫合", ["フィルタ"], ["構造", "色"], IsAviUtlSupported=false)]
public sealed class SaChromaticSutureEffect : VideoEffectBase {
 public override string Label => "色境界の縫合";
 [Display(Name="強さ", Description="原画と処理結果の混合率", Order=0)]
 [AnimationSlider("F1", "%", 0, 100)]
 public Animation Strength {get;} = new(92,0,100);
 [Display(Name="変化範囲", Description="構造が色を採取する最大距離の尺度", Order=1)]
 [AnimationSlider("F1", "px", 0, 256)]
 public Animation Radius {get;} = new(70,0,256);
 [Display(Name="縫い目の間隔", Description="構造の空間的な大きさ", Order=2)]
 [AnimationSlider("F1", "px", 4, 192)]
 public Animation Size {get;} = new(28,4,192);
 [Display(Name="しきい値", Description="小さな色差による反応を抑える", Order=3)]
 [AnimationSlider("F0", "", 0, 255)]
 public Animation Threshold {get;} = new(6,0,255);
 [Display(Name="縫い目の細かさ", Description="探索精度または模様の分割数。動画では固定を推奨", Order=4)]
 ...
 public override IVideoEffectProcessor CreateVideoEffect(IGraphicsDevicesAndContext devices) => new SaChromaticSutureProcessor(devices,this);
