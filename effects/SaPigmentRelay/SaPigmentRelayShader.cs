...
internal sealed class SaPigmentRelayShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaPigmentRelayShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaPigmentRelayShader).Assembly.GetManifestResourceStream("SaPigmentRelay.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
