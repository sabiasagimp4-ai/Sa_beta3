...
internal sealed class SaMembraneFoldShader(IGraphicsDevicesAndContext devices)
 : D2D1CustomShaderEffectBase(Create<SaMembraneFoldShader.Impl>(devices)) {
 ...
 private sealed class Impl : D2D1CustomShaderEffectImplBase<Impl> {
  ...
  static byte[] Load(){using var stream=typeof(SaMembraneFoldShader).Assembly.GetManifestResourceStream("SaMembraneFold.Shader.cso") ?? throw new InvalidOperationException("シェーダーがありません。");using var m=new System.IO.MemoryStream();stream.CopyTo(m);return m.ToArray();}
