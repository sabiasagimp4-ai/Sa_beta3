...
10個の独立したプロジェクト・クラス・名前空間・DLL。方式切替はありません。

追加5種（輪郭の空洞化・色の虹彩細胞・結晶のせり出し・色境界の縫合・色境界の繊毛）の仕様・インストールは [追加5種の説明](docs/structures/README.md)。新規5種だけのArtifactは **SaBeta3StructuresYmm**。以下は従来5種の説明。
...
| UI名 | 範囲 | 意味 |
|---|---|---|
| 強さ | 0–100% | 元画像との混合（仕上げの色味も含む） |
| 変化範囲 | 0–256px | 探索・変形距離の尺度 |
| 模様の幅 | 2–128px | 模様の繰り返し幅 |
| しきい値 | 0–255 | 小さい色差・勾配を抑制 |
| 探索回数 | 1–16 | 同じフレーム内の探索・加算回数 |
| 色相変化 | −180–180° | 構造反応に比例する色相回転 |
| 配置シード | 0–65535 | 空間配置を固定 |

従来5種の現在の初期値（C#の設定と対応）：

| DLL | 強さ | 変化範囲 | 模様の幅 | しきい値 | 探索回数 | 色相変化 | シード |
|---|---:|---:|---:|---:|---:|---:|---:|
| SaChromaticZipper | 78 | 42 | 20 | 10 | 16 | 28 | 7 |
| SaPetalGraft | 68 | 34 | 30 | 12 | 12 | 32 | 11 |
| SaChromaticBraid | 70 | 34 | 20 | 12 | 16 | 38 | 7 |
| SaMembraneFold | 76 | 46 | 28 | 8 | 16 | 26 | 7 |
| SaPigmentRelay | 70 | 34 | 28 | 14 | 12 | 30 | 17 |
...
[2026-09-13 全ブランチ監査と統合](docs/integration-20260913.md)。共通色処理とDirect2Dサンプラーを共有し、各DLL・UI・保存用クラスは独立したままです。
